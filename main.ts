...
import { BACKGROUND_COLOR, CAMERA_SETTINGS, SPHERE_NORMAL_URL, WATER_DISPLACEMENT_URL, 
  _FS_ripple_sphere, _FS_water, _VS_ripple_sphere, _VS_water } from './scene_settings';
...
if (skills) skills.onclick = () => {
  if (currentPage !== 'link-skills') {
    document.querySelector('#link-skills')?.scrollIntoView({
      behavior: 'smooth'
    });
    skills.focus();
    ...
    revertLink('link-skills');
    fadeToPageOpacity('landing-text', 100);
    fadeToPageOpacity('skills', 0);
    currentPage = 'landing-text';
    skills.textContent = skillsLabel;
  }
};
...
if (viewTitles) viewTitles.onclick = () => {
  document.querySelector('#google')?.scrollIntoView({
    behavior: 'smooth'
  });
  moveObjectTo(camera, getCamPos().add(new THREE.Vector3(0, -20, 0)));
}
